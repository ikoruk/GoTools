...
//===========================================================================
{
	...
	for (kj=0; kj<nmb_part_bd_cvs; ++kj)
	  {
	    if (ki == kj)
	      continue;
	    Point pos3 = 
	      part_bd_cvs[kj]->ParamCurve::point(part_bd_cvs[kj]->startparam());
	    Point pos4 = 
	      part_bd_cvs[kj]->ParamCurve::point(part_bd_cvs[kj]->endparam());
	...
	if (mindist <= eps)
	  part_bd_gap = std::max(part_bd_gap, mindist);
	if (min1 > eps)
	  part_bd_endpt.push_back(pos1);
	if (min2 > eps)
	  part_bd_endpt.push_back(pos2);
      }
	...
	for (size_t kr=0; kr<loose_endpt.size(); ++kr)
	  {
		...
		double par, dist;
		Point close;
		old_loop_cvs[kj]->closestPoint(loose_endpt[kr],
					       old_loop_cvs[kj]->startparam(),
					       old_loop_cvs[kj]->endparam(),
					       par, close, dist);
		if (dist < mindist)
	...
	if (nmb_int_pts > 0)
	  {
	    ...
	    double* pointpar1_2 = 0;
	    double* pointpar2_2 = 0;
	    s1859(sisl_sf1, sisl_sf2, epsco, epsgeo2, &nmb_int_pts,
		  &pointpar1, &pointpar2, &nmb_int_cvs, &intcurves, &status);
	    if (status < 0)
	      THROW("Failed intersecting surfs.");
	    if (pointpar1_2) free(pointpar1_2);
	    if (pointpar2_2) free(pointpar2_2);
	  }
...
//===========================================================================
{
    ...
    for (ki = 0; ki < int(new_cvs1.size()); ++ki) {
      double t1 = new_cvs1[ki]->startparam();
      double t2 = new_cvs1[ki]->endparam();
	Point start1 = new_cvs1[ki]->ParamCurve::point(t1);
	Point end1 = new_cvs1[ki]->ParamCurve::point(t2);
	if (start1.dist(end1) < epsge)
	  {
	    // Check mid point
	    Point mid1 = new_cvs1[ki]->ParamCurve::point(0.5*(t1+t2));
	...
	for (kj = 0; kj < int(new_cvs2.size()); ++kj) {
	  double t3 = new_cvs2[kj]->startparam();
	  double t4 = new_cvs2[kj]->endparam();
	  Point start2 = new_cvs2[kj]->ParamCurve::point(t3);
	  Point end2 = new_cvs2[kj]->ParamCurve::point(t4);
	    if (start2.dist(end2) < epsge)
	      {
		// Check mid point
		Point mid2 = new_cvs2[kj]->ParamCurve::point(0.5*(t3+t4));
	    ...
	    double len1_1 = 
	      new_cvs1[ki]->estimatedCurveLength(new_cvs1[ki]->startparam(), clo_t);
	    double len1_2 = 
	      new_cvs1[ki]->estimatedCurveLength(clo_t, new_cvs1[ki]->endparam());
	    ...
	    double len2_1 = 
	      new_cvs2[kj]->estimatedCurveLength(new_cvs2[kj]->startparam(), clo_t);
	    double len2_2 = 
	      new_cvs2[kj]->estimatedCurveLength(clo_t, new_cvs2[kj]->endparam());
	    if ((clo_dist < epsge && len2_1 > epsge && len2_2 > epsge) && 
		((clo_t - knot_diff_tol > new_cvs2[kj]->startparam()) &&
...
//===========================================================================
{
  ...
  for (size_t ki=0; ki<loop.size(); ++ki)
    {
      double cv_len = loop[ki]->estimatedCurveLength();
