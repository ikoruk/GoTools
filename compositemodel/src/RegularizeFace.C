...
//==========================================================================
  : epsge_(epsge), angtol_(angtol), tol2_(tol2), bend_(5.0*angtol), face_(face),
    ...
    top_level_(true), isolate_fac_(0.6), keep_vx_pri_(true), 
    allow_degen_(false), max_rec_(-1), curr_rec_(0)
{
...
//==========================================================================
  : face_(face), split_in_cand_(split_in_cand), split_mode_(1), 
    divideInT_(1), top_level_(true), isolate_fac_(0.6), keep_vx_pri_(true), 
    allow_degen_(false), max_rec_(-1), curr_rec_(0)
{
...
//==========================================================================
{
  ...
  // Check configuration for obsolete corners
  RegularizeUtils::checkCornerConfig(corners_, face_, 2.0*bend_);
...
//==========================================================================
{
  ...
  while (changed && (!found_cand_split))
      ...
      for (size_t ki=0; ki<sub_faces_.size(); ++ki)
	{
	  ...
	  // Add vertices prioritized for split
	  for (size_t kj=0; kj<vx_pri_.size(); ++kj)
	    {
	      size_t kr;
	      for (kr=0; kr<Tvx.size(); ++kr)
		if (vx_pri_[kj].get() == Tvx[kr].get())
		  break;
	      if (kr == Tvx.size())
		Tvx.push_back(vx_pri_[kj]);
	    }
	  ...
	  // Check configuration for obsolete corners
	  RegularizeUtils::checkCornerConfig(corner, face_, 2.0*bend_);

	  if ((corner.size() + Tvx.size() > 4 || vx_pri_.size() > 0) && 
	      divideInT_)
	    {
		  ...
		  !(faces.size() == 1 && faces[0].get() == face_.get()))
		{
		  ...
		  treated_faces_.insert(treated_faces_.end(), faces.begin(),
					faces.end());
		  changed = true;
		}
	    }
...
{
  vector<shared_ptr<ftSurface> > faces;
  if (Tvx.size() + corner.size() <= 4 && vx_pri_.size() == 0)
  ...
  for (ki=0; ki<Tvx.size(); ++ki)
      ...
      for (ka=0; ka<Tvx.size(); ++ka)
	{
	  if (ka == ki)
	    continue;
	  if (Tvx[ki]->sameEdge(Tvx[ka].get(), 0.0))
	    break;
	}
  ...
  ftEdge* cand_edge = 0;
  // Remove vertices that mess up the structure
  RegularizeUtils::checkCornerConfig(vx, face_, 2.0*bend_);  
  ...
  bool strong = (allow_degen_ || corner.size() < 4);
  ...
  if (allow_degen_)
      ...
      else if (deg_face_.get())
	{
	  ...
	  vector<shared_ptr<Vertex> > other_corners = 
	    deg_face_->getCornerVertices(bend_, 0);

	  // Find opposite corner (if close)
	  int level = 3;
	  int min_level = -1;
	  size_t kh;
	  for (kh=0; kh<other_corners.size(); ++kh)
	    {
	      if (curr->sameEdge(other_corners[kh].get(), angtol_, true))
		{
		  level = 1;
		  min_level = (int)kh;
		}
	      else if (level > 1 && 
		       curr->connectedToSameVertex(other_corners[kh].get(), 
						   angtol_, true))
		{
		  level = 2;
		  min_level = (int)kh;
	  ...
	  if (level <= 2)
	    {
	      // Identify corners with the same level of connection
    ...
    {
      cand_vx.insert(cand_vx.begin(), Tvx.begin(), Tvx.end());
      Tvx.clear();
    }
...
//==========================================================================
{
  ...
  for (ki=0; ki<vx.size(); )
    {
      size_t kj;
      for (kj=0; kj<connect.size(); ++kj)
	{
	  if (connect[kj].first.get() == vx[ki].get() ||
	      connect[kj].second.get() == vx[ki].get())
	    break;
	}
      if (ki < connect.size())
	{
	  vx.erase(vx.begin()+ki);
	  vx_match.erase(vx_match.begin()+ki);
	  if (ki < nmb_Tvx)
	    nmb_Tvx--;
	}
      else
  ...
  for (ki=0; ki<connect.size(); ++ki)
    {
      // First check if any candidates should be moved
      shared_ptr<Vertex> vx1 = connect[ki].first;
      shared_ptr<Vertex> vx2 = connect[ki].second;
      if (vx2.get())
	{
	  Point in_vec1 = RegularizeUtils::getInVec(vx1, face);
	  Point in_vec2 = RegularizeUtils::getInVec(vx2, face);
	  bool single1 = true, single2 = true;
	  for (size_t kj=0; kj<connect.size(); ++kj)
	    {
	      if (kj == ki)
		continue;
	      if (connect[kj].first.get() == vx1.get() ||
		  connect[kj].second.get() == vx1.get())
		single1 = false;
	      if (connect[kj].first.get() == vx2.get() ||
		  connect[kj].second.get() == vx2.get())
		single2 = false;
	    }

	  Point vec = vx2->getVertexPoint() - vx1->getVertexPoint();
	  if (vec*in_vec1 < -fac2*angtol_ && single2)
	    {
	      bool modified = 
		RegularizeUtils::updateVertexPos(face, vx1, vx2, epsge_,
						 tol2_, angtol_);
	      if (modified)
		connect[ki].second = vx2;
	    }
	  
	  if (vec*in_vec2 < -fac2*angtol_ && single1)
	    {
	      bool modified = 
		RegularizeUtils::updateVertexPos(face, vx2, vx1, epsge_,
						 tol2_, angtol_);
	      if (modified)
		connect[ki].first = vx1;
	    }
	}
      ...
      if (vx2.get()) 
	pcurve = RegularizeUtils::checkStrightParCv(face, vx1, vx2,
						    epsge_, bend_);
      if (pcurve.get())
	{
	  seg = BoundedUtils::getTrimCrvsPcrv(surface, pcurve, epsge_, bd_sf);
	}
      else
	{
	  Point vx_par1 = vx1->getFacePar(face.get());
	  Point vx_par2 = (vx2.get()) ?
	    vx2->getFacePar(face.get()) : common_par;
	  seg = BoundedUtils::getTrimCrvsParam(surface, vx_par1, vx_par2, 
					       epsge_, bd_sf);
	}
...
//==========================================================================
{
  ...
  if (concave_corners.size() > 0)
   ...
   // Fetch T-joint vertices 
   vector<shared_ptr<Vertex> > Tvx = getTjointVertices();

   // Check for a symmetric situation 
   // Should Tjoints and prioritized vertices be considered?
   if (vx_pri_.size() == 0 && Tvx.size() == 0 &&
       corners.size() > 5 && corners.size() % 2 == 0 && top_level_)
     {
       subfaces = splitBetweenCorners(corners);
       if (subfaces.size() > 1)
	 return subfaces;
     }
...
#ifdef DEBUG_REG
   std::ofstream of02("Tvx.g2");
   of02 << "400 1 0 4 255 0 0 255" << std::endl;
   of02 << Tvx.size() << std::endl;
   for (ki=0; ki<Tvx.size(); ++ki)
   ...
   if (vx_pri_.size() > 0)
     {
       vector<shared_ptr<Vertex> > sorted_pri = prioritizeCornerVx(vx_pri_,
								   true);
       sorted_corners.insert(sorted_corners.begin(), sorted_pri.begin(), 
			     sorted_pri.end());
     }
  ...
  for (ki=0; ki<sorted_corners.size(); ++ki)
       ...
       for (size_t kr=0; kr<half_holes.size(); ++kr)
	 {
       ...
       for (size_t kr=0; kr<vx_pri_.size(); ++kr)
       	 {
	   if (vx_pri_[kr].get() == split_vx.get())
	     continue;

       	   size_t kh;
       	   for (kh=0; kh<cand_vx.size(); ++kh)
       	     if (cand_vx[kh].get() == vx_pri_[kr].get())
       	       {
       		 prio.push_back(cand_vx[kh]);
       		 cand_vx.erase(cand_vx.begin()+kh);
       		 break;
	   ...
	   if (kh == cand_vx.size())
	     prio.push_back(vx_pri_[kr]);
       	 }
       for (size_t kr=0; kr<Tvx.size(); ++kr)
       	 {
	   if (Tvx[kr].get() == split_vx.get())
	     continue;

       	   size_t kh;
       	   for (kh=0; kh<cand_vx.size(); ++kh)
       	     if (cand_vx[kh].get() == Tvx[kr].get())
       	       {
       		 prio.push_back(cand_vx[kh]);
       		 cand_vx.erase(cand_vx.begin()+kh);
       		 break;
       ...
       // Check if any vertices are locked for connection with the
       // identified split vertex
       for (size_t kr=0; kr<no_connect_vxs_.size(); ++kr)
	 {
	   size_t kh;
	   for (kh=0; kh<no_connect_vxs_[kr].size(); ++kh)
	     {
	       if (no_connect_vxs_[kr][kh].get() == split_vx.get())
		 break;
	     }
	   if (kh < no_connect_vxs_[kr].size())
	     {
	       // Check if any vertices must be removed from the
	       // list of candidates
	       for (kh=0; kh<no_connect_vxs_[kr].size(); ++kh)
		 {
		   for (size_t ka=0; ka<cand_vx.size(); ++ka)
		     {
		       if (no_connect_vxs_[kr][kh].get() == cand_vx[ka].get())
			 {
			   cand_vx.erase(cand_vx.begin()+ka);
			   break;
			 }
		     }
		 }
       ...
       if (allow_degen_ && deg_face_.get())
	 {
	   // Prioritize corners opposite to corners of the degenerate face
	  vector<shared_ptr<Vertex> > other_corners = 
	    deg_face_->getCornerVertices(bend_, 0);
	   
	  // Find logical distance between the split vertex and the 
	  // degenerate face
	  int level = 3;
	  int min_level = -1;
	  size_t kh;
	  for (kh=0; kh<other_corners.size(); ++kh)
	    {
	      if (split_vx->sameEdge(other_corners[kh].get(), angtol_, true))
		{
		  level = 1;
		  min_level = (int)kh;
		}
	      else if (level > 1 && 
		       split_vx->connectedToSameVertex(other_corners[kh].get(), 
						       angtol_, true))
		{
		  level = 2;
		  min_level = (int)kh;
	  ...
	  if (level <= 2)
	    {
	      // Identify corner with the same level of connection
	      for (kh=0; kh<cand_vx.size(); ++kh)
		{
		  size_t kr;
		  int first_found = -1;
       ...
       if (cand_split_.size() > 0)
	 {
       ...
       if (!allow_degen_)
	 {
	     vector<shared_ptr<Vertex> > corners2 = 
	       face_->getCornerVertices(bend_, 0);
...
//==========================================================================
{
  ...
  // Identify edge opposite to vertex if the face has an odd number of
  // edges
  DirectionCone dummy;  
  vector<vector<ftEdge*> > opposite_edge(cand_vx2.size());
  vector<DirectionCone> cone(cand_vx2.size(), dummy);
  vector<double> len(cand_vx2.size());
  vector<shared_ptr<ftEdge> > face_edges = face_->getAllEdges();
  Body *bd = face_->getBody();
  size_t nmb = cand_vx2.size()/2;
  size_t kj, kr;
  if (!reduced && (int)cand_vx2.size()%2 == 1)
    {
      for (size_t ki=0; ki<cand_vx2.size(); ++ki)
	{
	  // Identify the opposite edge(s)
	  shared_ptr<Vertex> prev_vx = cand_vx2[(ki+nmb)%cand_vx2.size()];
	  shared_ptr<Vertex> next_vx = cand_vx2[(ki+nmb+1)%cand_vx2.size()];

	  opposite_edge[ki] = Path::edgeChainFace(face_.get(), prev_vx, 
						  next_vx, cand_vx2[ki]);
	}
    }
  else if (reduced && (int)face_edges.size()%2 == 1)
    {
      for (size_t ki=0; ki<cand_vx2.size(); ++ki)
	{
	  // Identify the opposite edge
	  ftEdge* curr_opposite;
	  shared_ptr<Vertex> curr_vx = cand_vx2[ki];
	  shared_ptr<Vertex> prev_vx;
	  int nmb = (int)face_edges.size()/2;
	  vector<shared_ptr<Vertex> > next_vx;
	  for (int kk=0; kk<nmb; ++kk)
	    {
	      next_vx = curr_vx->getNextVertex(face_.get());
	      kr = 0;
	      if (prev_vx.get())
		{
		  for (kr=0; kr<next_vx.size(); ++kr)
		    if (next_vx[kr].get() != prev_vx.get())
		      break;
		}
	      if (kr < next_vx.size())
		{
		  ...
		  vector<ftEdge*> curr_edges = 
		    curr_vx->getFaceEdges(face_.get());
		  for (kj=0; kj<curr_edges.size(); ++kj)
		    if (!prev_vx->hasEdge(curr_edges[kj]))
		      {
			curr_opposite = curr_edges[kj];
			break;
		      }
		}
	    }
	  opposite_edge[ki].push_back(curr_opposite);
	}
    }

  for (size_t ki=0; ki<opposite_edge.size(); ++ki)
    {
      if (opposite_edge[ki].size() == 0)
	continue;
      cone[ki] = opposite_edge[ki][0]->geomCurve()->directionCone();
      len[ki] = opposite_edge[ki][0]->estimatedCurveLength();
      for (kr=1; kr<opposite_edge[ki].size(); ++kr)
	{
	  DirectionCone cone2 = 
	    opposite_edge[ki][kr]->geomCurve()->directionCone();
	  cone[ki].addUnionWith(cone2);
	  double len2 = opposite_edge[ki][kr]->estimatedCurveLength();
	  len[ki] += len2;
	}
    }

  if (cand_split_.size() > 0 && (split_in_cand_ || cand_vx2.size() == 5))
    {
      // VSK 0517. Compare with cand_split_ vertices only when split_in_cand_
      // is set. This might create problems, but the opposite creates problems
      // as well. A better restriction might be required
      ...
      for (ki=0; ki<cand_vx.size(); ++ki)
	{
      ...
      // Compute also minimum distance between split points in the 
      // corresponding face
      double min_dist2 = 1.0e8;
      for (ki=0; ki<cand_split_.size(); ++ki)
	{
	  double dist = 
	    cand_split_[ki].first.first.dist(cand_split_[ki].second.first);
	  min_dist2 = std::min(min_dist2, dist);
	}
      double level_dist = std::min(0.5*min_dist2, 0.2*min_dist);
      ...
      for (ki=0; ki<cand_split_.size(); ++ki)
	{
      ...
      // Assemble vertices in already treated faces
      set<shared_ptr<Vertex> > corr_vx;
      for (ki=0; ki<treated_faces_.size(); ++ki)
	{
	  vector<shared_ptr<Vertex> > tmp_vxs = 
	    treated_faces_[ki]->vertices();
	  corr_vx.insert(tmp_vxs.begin(), tmp_vxs.end());
	}
      vector<shared_ptr<Vertex> > corr_vx2(corr_vx.begin(), corr_vx.end());
      ...
      for (ki=0; ki<corr_pts.size(); ++ki)
	{
	  ...
	  // Check for a direct correspondance. Find vertex corresponding
	  // to point
	  double min_dist2 = min_dist;
	  int min_ind = -1;
	  for (kj=0; kj<corr_vx2.size(); ++kj)
	    {
	      double dist = 
		corr_vx2[kj]->getVertexPoint().dist(corr_pts[ki].first);
	      if (dist < min_dist2)
		{
		  min_dist2 = dist;
		  min_ind = (int)kj;
	  ...
	  for (kj=nmb_pri; kj<cand_vx2.size(); ++kj)
	    {
	      if (cand_vx2[kj]->sameEdge(corr_vx2[min_ind].get(), 
					 angtol_, true))
		{
	  	  std::swap(cand_vx2[nmb_pri], cand_vx2[kj]);
	  	  ++nmb_pri;
		  break;
		}
	    }
	  ...
	  // if (min_dist2 < level_dist)
	  //   {
	  //     // Check if a non-corner has a smaller distance
	  //     for (kj=0; kj<non_corner.size(); ++kj)
	  // 	{
	  // 	  double dist = pos1.dist(non_corner[kj]->getVertexPoint());
	  // 	  if (dist < min_dist2)
	  // 	    break;
	  // 	}
	  //     if (kj == non_corner.size())
	  // 	{
	  // 	  std::swap(cand_vx2[nmb_pri], cand_vx2[min_ind]);
	  // 	  ++nmb_pri;
	  // 	}
	  //   }
	}
  ...
  if (nmb_pri == 0 && cand_vx2.size() >= 4)
    {
      // Prioritize corners where one adjacent face is triangular
      for (size_t ki=0; ki<cand_vx2.size(); ++ki)
	{
	  vector<ftSurface*> vx_faces = cand_vx2[ki]->faces(bd);
	  for (size_t kj=0; kj<vx_faces.size(); ++kj)
	    {
	      int nmb_corners = vx_faces[kj]->nmbCornerVertices(bend_);
	      if (nmb_corners == 3)
		{
		  std::swap(cand_vx2[nmb_pri], cand_vx2[ki]);
		  ++nmb_pri;
		  break;
		}
	    }
  ...
  if (nmb_pri == 0 && cand_vx2.size() == 5)
    {
      // Special case. Preferably a corner is joined to the
      // interior of the opposite edge
      // Prioritize corners where the adjacent surface
      // to the opposite edge has not been regularized before
      // Prioritize also faces that have a corner where only two 
      // faces meet
      for (size_t ki=0; ki<cand_vx2.size(); ++ki)
	{
	  vector<ftSurface*> vx_faces = cand_vx2[ki]->faces(bd);
	  std::set<shared_ptr<ParamSurface> > under_sf;
	  for (size_t kj=0; kj<vx_faces.size(); ++kj)
	    {
	      shared_ptr<ParamSurface> tmp_sf = vx_faces[kj]->surface();
	      shared_ptr<BoundedSurface> tmp_bd = 
		dynamic_pointer_cast<BoundedSurface,ParamSurface>(tmp_sf);
	      if (tmp_bd.get())
		tmp_sf = tmp_bd->underlyingSurface();
	      under_sf.insert(tmp_sf);
	    }
	  int nmb_under = (int)under_sf.size();
	  ftSurface *adj_face = NULL;
	  if (opposite_edge[ki].size() == 1 && opposite_edge[ki][0]->twin())
	    adj_face = opposite_edge[ki][0]->twin()->face()->asFtSurface();
	  if (adj_face)
	    {
	      // Check if the adjacent face already has been subject
	      // to regularization
	      for (kr=0; kr<treated_faces_.size(); ++kr)
		{
		  if (adj_face == treated_faces_[kr].get())
		    break;
		}
	    }
	  if ((adj_face && kr == treated_faces_.size()) || 
	      nmb_under == 2)
	    {
	      std::swap(cand_vx2[nmb_pri], cand_vx2[ki]);
	      std::swap(len[nmb_pri], len[ki]);
	      std::swap(cone[nmb_pri], cone[ki]);
	      std::swap(opposite_edge[nmb_pri], opposite_edge[ki]);
	      nmb_pri++;
	    }
	}
      ...
      if (nmb_pri == 0)
	{
	  for (size_t ki=0; ki<cand_vx2.size(); ++ki)
		...
		{
		  vector<shared_ptr<Vertex> > Tvx = 
		    getTjointVertices(adj_face);
		  
		  // Remove T vertices being connected to the edge vertices
		  shared_ptr<Vertex> vx1, vx2;
		  opposite_edge[ki][0]->getVertices(vx1, vx2);
		  for (int kh=(int)Tvx.size()-1; kh>=0; --kh)
		    {
		      if (Tvx[kh]->sameEdge(vx1.get()) || 
			  Tvx[kh]->sameEdge(vx2.get()))
			Tvx.erase(Tvx.begin()+kh);
		    }
      ...
      // Sort subgroups with respect to decreasing angle in direction cone
      int kk=0, kh=0;
      for (kk=0; kk<nmb_pri; ++kk)
	for (kh=kk+1; kh<nmb_pri; ++kh)
	  if (cone[kh].angle() > cone[kk].angle())
	  {
	    std::swap(cand_vx2[kk], cand_vx2[kh]);
	    std::swap(cone[kk], cone[kh]);
	    std::swap(len[kk], len[kh]);
	    std::swap(opposite_edge[kk], opposite_edge[kh]);
	  }

      for (kk=nmb_pri; kk<(int)cand_vx2.size(); ++kk)
	for (kh=kk+1; kh<(int)cand_vx2.size(); ++kh)
	  if (cone[kh].angle() > cone[kk].angle())
	  {
	    std::swap(cand_vx2[kk], cand_vx2[kh]);
	    std::swap(cone[kk], cone[kh]);
  ...
  // Use the angle internally to the face
  vector<double> angle(cand_vx2.size());
  vector<double> min_len(cand_vx2.size(), 0.0);
  vector<double> len_frac(cand_vx2.size(), 1.0);
  vector<int> perpendicular(cand_vx2.size(), 0);
  double tol = 1.0e-4;
  // VSK, 070817. Remove modified tolerance in the 5-sided case to
  // check out where it makes sense
  //double ang_tol = (cand_vx2.size() == 5) ? 0.25*M_PI : 0.001;
...
// 	tan2 *= -1;
	...
	ang = 2.0*M_PI - ang;
      ...
      if (opposite_edge[ki].size() == 1)
	{
	  double par, dist;
	  Point close_pt;
	  opposite_edge[ki][0]->closestPoint(cand_vx2[ki]->getVertexPoint(), 
					     par, close_pt, dist);
	  double len1 = 
	    opposite_edge[ki][0]->estimatedCurveLength(opposite_edge[ki][0]->tMin(),
						    par);
	  double len2 = 
	    opposite_edge[ki][0]->estimatedCurveLength(par,
						    opposite_edge[ki][0]->tMax());
	  min_len[ki] = std::min(len1, len2);
	  len_frac[ki] = min_len[ki]/len[ki];
	}
    }


  // Sort vector according to decreasing angle or length fraction
  // depending on whether we sort a reduced set or not, but prioritize 
  // selected vertices
      int kk, kh;
      for (kk=0; kk<nmb_pri; ++kk)
	for (kh=kk+1; kh<nmb_pri; ++kh)
	  {
	    bool swap = false;
	    if (reduced && len_frac[kh] > len_frac[kk])
	      swap = true;
	    else if ((reduced && fabs(len_frac[kh]-len_frac[kk]) < ang_tol) ||
		     (!reduced))
	      {
		if (angle[kh] > angle[kk]+ang_tol ||
		    (fabs(angle[kh]-angle[kk]) < tol && perpendicular[kk] &&
		     (!perpendicular[kh])))
		  swap = true;
	      }
	    if (swap)
...
//==========================================================================
{
  ...
  for (ki=0; ki<edges.size(); ++ki)
    {
      ...
      av_edge_len += edges[ki]->geomEdge()->estimatedCurveLength();
    }
  av_edge_len /= (int)edges.size();
  ...
  for (ki=0; ki<corner_ind.size(); ++ki)
	...
	arc_len.push_back(arc->estimatedCurveLength(3));
    }
      ...
      for (ki=0; ki<cone_ang.size(); ++ki)
	{
      ...
      // 	{
      	  if (/*cone_ang[ki] < ang_limit && */ edge_ang > opening_limit &&
      	      /* VSK 1208 I am not sure about this, but try it to avoid false
      		 half holes */ fabs(M_PI-edge_ang) > fac2*opening_limit &&
  ...
  if (select_vx.get())
      ...
      for (ki=0; ki<select_end.size(); )
	{
	  if (select_vx->sameEdge(select_end[ki].get()) ||
	      select_vx->connectedToSameVertex(select_end[ki].get(), 0.0))
...
{
#ifdef DEBUG_REG
  std::ofstream of_cand("cand_vx_split.g2");
  of_cand << "400 1 0 4 255 0 0 255" << std::endl;
  of_cand << vx.size() << std::endl;
  for (size_t ki=0; ki<vx.size(); ++ki)
    of_cand << vx[ki]->getVertexPoint() << std::endl;
#endif

  cand_edge = 0;
  double bend2 = 2.0*bend_;
  for (size_t ki=0; ki<vx.size(); ++ki)
    {
      ...
      for (kj=0; kj<edges.size(); ++kj)
	{
	  ...
	  if (other->sameEdge(select_vx.get(), bend2, true) && !allow_degen_)
	    break;

	  shared_ptr<Vertex> common = 
	    select_vx->connectionVertex(other.get(), bend2, true);
	  if (common.get() &&  !allow_degen_)
	    {
	      if (other->nmbUniqueEdges() == 2)
		{
		  vector<ftEdge*> edg2 = other->uniqueEdges();
		  double t1 = edg2[0]->parAtVertex(other.get());
		  double t2 = edg2[1]->parAtVertex(other.get());
		  Point tan1 = edg2[0]->tangent(t1);
		  Point tan2 = edg2[1]->tangent(t2);
		  if (tan1.angle(tan2) < bend2)
	      ...
	      if (common->nmbUniqueEdges() == 2)
		{
		  ...
		  double t1 = edg2[0]->parAtVertex(common.get());
		  double t2 = edg2[1]->parAtVertex(common.get());
		  Point tan1 = edg2[0]->tangent(t1);
		  Point tan2 = edg2[1]->tangent(t2);
		  if (tan1.angle(tan2) < bend2)
		    break;  // Not a significant vertex
...
//==========================================================================
{
  ...
  for (ki=0, kr=0; ki<half_holes.size(); ++ki, ++kr)
      ...
      for (kj=0; kj<edges.size(); ++kj)
	{
      ...
      curr = edges[edges.size()-1]->geomEdge();
...
//==========================================================================
{
  ...
  for (size_t ki=0; ki<vx.size(); ++ki)
      ...
      for (size_t kj=0; kj<edges.size(); kj=kr)
	{
	  ...
	  bool same_elem = false;
	  if (faces.size() == 3)
	    {
	      // Check underlying surface
	      vector<ftSurface*> faces2(faces.begin(), faces.end());
	      vector<shared_ptr<ParamSurface> > under;
	      size_t kh;
	      for (kh=0; kh<faces2.size(); )
		{
		  if (faces2[kh] == face_.get())
		    faces2.erase(faces2.begin()+kh);
		  else
		    {
		      under.push_back(faces2[kh]->surface());
		      ++kh;
		    }
		}
	      for (kh=0; kh<under.size(); ++kh)
		{
		  shared_ptr<BoundedSurface> bd_sf = 
		    dynamic_pointer_cast<BoundedSurface, ParamSurface>(under[kh]);
		  if (bd_sf.get())
		    under[kh] = bd_sf->underlyingSurface();
		}

	      if (under.size() == 2)
		same_elem = SurfaceModelUtils::sameElementarySurface(under[0], 
								     under[1],
								     epsge_,
								     angtol_);

	      if (faces2.size() != 2)
		same_elem = false;  // Unexpected

	      if (same_elem)
		{
		  // Exclude plane. Does not have a seam
		  shared_ptr<ParamSurface> parent = under[0]->getParentSurface();
		  if (parent.get())
		    {
		  ...
		  if (same_elem)
		    {
		      // Check if the edge between the rotational faces continues
		      vector<shared_ptr<Vertex> > common_vx = 
			faces2[0]->getCommonVertices(faces2[1]);

		      // Remove current
		      for (kh=0; kh<common_vx.size();)
			{
			  if (common_vx[kh].get() == vx[ki].get())
			    common_vx.erase(common_vx.begin()+kh);
			  else 
			    ++kh;
			}
		      if (common_vx.size() != 1)
			same_elem = false;  // Unexpected
		      ...
		      vector<ftSurface*> other_faces = 
			common_vx[0]->faces(body);
		      if (other_faces.size() != 3)
			  same_elem = false;
		    }
	    }

	  if (faces.size() != 2 && !same_elem)
	    seam = false;
...
//==========================================================================
vector<shared_ptr<ftSurface> >
RegularizeFace::splitBetweenCorners(vector<shared_ptr<Vertex> >& corners)
//==========================================================================
{
  vector<shared_ptr<ftSurface> > subfaces;

  // Look for a line along which the vertices can be sorted
  size_t ki, kj, kr;
  int ka;
  double lim_ang = 2.0*bend_; //M_PI/4.0;
  int nmb = (int)corners.size();
  vector<double> parval(nmb);
  vector<double> cornerdist(nmb);
  double tmin = -1.0e8;
  double tmax = 1.0e8;
  for (ki=0; ki<corners.size(); ++ki)
    {
      DirectionCone cone1;
      for (kj=ki, ka=0; ka<nmb/2-1; kj=(kj+1)%nmb, ++ka)
	{
	  kr = (kj+1)%nmb;
	  Point dir = corners[kr]->getVertexPoint() - 
	    corners[kj]->getVertexPoint();
	  if (cone1.dimension() == 0)
	    cone1 = DirectionCone(dir);
	  else
	    cone1.addUnionWith(dir);
	}
      DirectionCone cone2;
      for (kj=(kr+1)%2, ka=0; ka<nmb/2-1; kj=(kj+1)%nmb, ++ka)
	{
	  kr = (kj+1)%nmb;
	  Point dir = corners[kj]->getVertexPoint() - 
	    corners[kr]->getVertexPoint();
	  if (cone2.dimension() == 0)
	    cone2 = DirectionCone(dir);
	  else
	    cone2.addUnionWith(dir);
	}
      cone1.addUnionWith(cone2);
      if (cone1.greaterThanPi() || cone1.angle() > lim_ang)
	continue;

      Point pos = 0.5*(corners[ki]->getVertexPoint()+
		       corners[(ki+nmb/2)%nmb]->getVertexPoint());
      Line line(pos, cone1.centre());

      // Project all corners onto the line and not signed distance and
      // parameter value
      Point vec = corners[ki]->getVertexPoint() - pos;
      for (kj=0; kj<corners.size(); ++kj)
	{
	  double par, dist;
	  Point clo_pt;
	  Point vx_pos = corners[kj]->getVertexPoint();
	  line.closestPoint(vx_pos, tmin, tmax,
			    par, clo_pt, dist);
	  parval[kj] = par;
	  int sgn = ((vx_pos - clo_pt)*vec < 0.0) ? -1 : 1;
	  cornerdist[kj] = sgn*dist;
	}

      // Count number of sign changes and direction changes
      int nmbsgn1 = 0;
      int nmbsgn2 = 0;
      kj = (ki+1)%nmb;
      double dd1 = parval[kj] - parval[ki];
      double dd2 = cornerdist[ki];
      for (kr=(kj+1)%nmb, ka=1; ka<nmb; kj=kr, kr=(kj+1)%nmb, ++ka)
	{
	  double dd3 = parval[kr]-parval[kj];
	  double dd4 = cornerdist[kj];
	  if (dd1*dd3<0.0 && ka<nmb-1)
	    nmbsgn1++;
	  if (dd2*dd4 < 0.0)
	    nmbsgn2++;
	  dd1 = dd3;
      ...
      if (nmbsgn1 != 1 || nmbsgn2 != 1)
	continue;

      // Define division curves
      shared_ptr<ParamSurface> surf = face_->surface();
      shared_ptr<BoundedSurface> bd_sf;
      vector<shared_ptr<CurveOnSurface> > trim_segments;
      int nmb_diff = nmb-3;
      for (ka=1; ka<nmb/2-1; ++ka)
	{
	  shared_ptr<Vertex> vx1 = corners[(ki+ka)%nmb];
	  shared_ptr<Vertex> vx2 = corners[(ki+ka+nmb_diff)%nmb];
	  nmb_diff -= 2;
	  shared_ptr<ParamCurve> pcurve = 
	    RegularizeUtils::checkStrightParCv(face_, vx1, vx2,
					       epsge_, angtol_);
	  vector<shared_ptr<CurveOnSurface> > curr_seg;
	  if (pcurve.get())
	    {
	      curr_seg = BoundedUtils::getTrimCrvsPcrv(surf, pcurve, epsge_,
						       bd_sf);
	    }
	  else
	    {
	      Point parval1 = vx1->getFacePar(face_.get());
	      Point parval2 = vx2->getFacePar(face_.get());
	      curr_seg = BoundedUtils::getTrimCrvsParam(surf, parval1,
							parval2, epsge_,
							bd_sf);
	    }
	  ...
	  for (size_t kh=0; kh<curr_seg.size(); ++kh)
	    {
	      shared_ptr<ParamCurve> cv = curr_seg[kh]->spaceCurve();
	  ...
	  // Check current segments
	  vector<shared_ptr<Vertex> > next_vxs = 
	    vx1->getNextVertex(face_.get());
	  vector<shared_ptr<Vertex> > next2 =
	    vx2->getNextVertex(face_.get());
	  next_vxs.insert(next_vxs.end(), next2.begin(), next2.end());
      
	  double curr_tol = 
	    0.05*vx1->getVertexPoint().dist(vx2->getVertexPoint());
	  if (curr_seg.size() > 0)
	    RegularizeUtils::checkTrimSeg(curr_seg, next_vxs, 
					  vx1->getVertexPoint(), 
					  vx2->getVertexPoint(), 
...
#ifdef DEBUG_REG
      std::ofstream out_file("split_segments.g2");
      for (size_t kh=0; kh<trim_segments.size(); ++kh)
	{
	  shared_ptr<ParamCurve> cv = trim_segments[kh]->spaceCurve();
	  cv->writeStandardHeader(out_file);
	  cv->write(out_file);
	}
#endif
...
#ifdef DEBUG_REG
      std::ofstream of("split_surf.g2");
      for (size_t kh=0; kh<sub_sfs.size(); ++kh)
	{
	  sub_sfs[kh]->writeStandardHeader(of);
	  sub_sfs[kh]->write(of);
	}
#endif
...
//==========================================================================
vector<shared_ptr<Vertex> >
RegularizeFace::getTjointVertices(ftSurface *face)
//==========================================================================
{
  // Get potensial T-joints
  ftSurface *curr_face = (face) ? face : face_.get();
  Body *curr_body = curr_face->getBody();
  vector<shared_ptr<Vertex> > Tvx = 
    curr_face->getNonCornerVertices(bend_ /*angtol_*/, 0);
  ...
  for (int kj=0; kj<(int)Tvx.size(); )
      ...
      else
	{
	  ...
	  // corner of this face
	  ftSurface *other = (vx_faces[0] == curr_face) ?
...
//==========================================================================
{
  if (face_->hasBoundaryConditions())
    {
      int bd_type, bd;
      face_->getBoundaryConditions(bd_type, bd);
  ...
  for (int kj=0; kj<nmb_faces; )
    {
      RegularizeFace regularize(new_faces[kj], model_, split_in_cand_);
	...
	regularize.setCandSplit(cand_split_);
      ...
      if (vx_pri_.size() > 0 && keep_vx_pri_)
	{
	  vector<shared_ptr<Vertex> > vxpri;
	  vector<shared_ptr<Vertex> > vx = new_faces[kj]->vertices();
	  for (size_t kr=0; kr<vx_pri_.size(); ++kr)
	    {
	      size_t kh;
      ...
      if (treated_faces_.size() > 0)
	regularize.setTreated(treated_faces_);

      vector<shared_ptr<ftSurface> > faces2 = 
	regularize.getRegularFaces();

      treated_faces_.insert(treated_faces_.end(), 
			    faces2.begin(), faces2.end());

      if (faces2.size() > 1)
	{
	  new_faces.erase(new_faces.begin()+kj);
	  nmb_faces--;
	  new_faces.insert(new_faces.end(), faces2.begin(), faces2.end());

	  // Check if any new faces may be joined across the seam
	  mergeSeams(new_faces, nmb_faces, faces2);
	}
      else
	kj++;
    }
