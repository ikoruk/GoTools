  ...
  if (allow_degen_)
      ...
      else if (deg_face_.get())
	{
	  ...
	  for (kh=0; kh<other_corners.size(); ++kh)
	    {
	      if (curr->sameEdge(other_corners[kh].get()))
		{
		  level = 1;
		  min_level = (int)kh;
		}
	      else if (level > 1 && curr->connectedToSameVertex(other_corners[kh].get()))
		{
		  level = 2;
		  min_level = (int)kh;
		}
